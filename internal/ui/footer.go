...
func renderFooter(s Styles, m model) string {
	...
	case m.focus == focusNotes:
		...
		case m.daily.cursor == mc:
			hints = []string{"tab goals", "i write", "e $EDITOR", "↑↓ scroll", "[ ] day", "w refresh", "q quit"}
		case m.daily.cursor == nn || m.daily.cursor == nn+1:
			hints = []string{"tab goals", "1-5 set rating", "↑↓ move", "[ ] day", "q quit"}
		default:
			hints = []string{"tab goals", "space toggle", "↑↓ move", "[ ] day", "q quit"}
		}
	default: // focusGoals
		hints = []string{"tab journal", "↑↓ move", "space toggle", "a add", "e edit", "d del", "[ ] day", "w refresh", "q quit"}
	}
